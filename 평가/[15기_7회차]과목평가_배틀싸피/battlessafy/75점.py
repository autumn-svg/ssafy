...
# 전략
# 목표물이 사정 거리 안에 들어오면 추가 이동 없이 쏘고, 물이 아닐 때 이동 하는 조건, 메가탄 쏘는 것을 추가함
...
def bfs(grid, start, target, wall):
    ...
    while queue:
        (r, c), actions = queue.popleft() # (r,c) = (row,col)

        for d, (dr, dc) in enumerate(DIRS): # (dr, dc) = (0,1), (1,0), (0,-1), (-1,0)
            nr, nc = r + dr, c + dc # 큐에서 팝한 위치에서 DIRS 방향으로 이동한 위치
            # 사정거리인 3 안에 들어오면 쏜다
            for m in range(1,4): # 사정거리
                ni = r + dr * m
                nj = c + dc * m
                if 0 <= ni < rows and 0 <= nj < cols:
                    if grid[ni][nj] == wall: # 벽 만나면 부수고 다시
                        break
                    if (ni, nj) == target: # 조준해야 할 목표가 사정거리 안에 들어오면
                        return actions + [FIRE_CMDS[d]] # 행동하고(방향) 쏜다
                    elif (ni, nj) == target: # 조준해야 할 목표가 사정거리 안에 들어오면
                        return actions + [M_FIRE_CMDS[d]] # 메가탄 쏜다
                    elif grid[ni][nj] == 'T':
                        return actions + [M_FIRE_CMDS[d]] # 메가탄 쏜다
            if (nr, nc) == target: # 조준해야 할 목표와 이동한 좌표가 같으면
                return actions + [FIRE_CMDS[d]] # 행동하고(방향) 쏜다
        ...
        for d, (dr, dc) in enumerate(DIRS):
            ...
            if 0 <= nr < rows and 0 <= nc < cols and grid[nr][nc] != wall and (nr, nc) not in visited and grid[nr][nc] != 'W' : 
                # (r + dr)이 0보다 크거나 같고 len(grid) 보다 작고, c + dc가 0보다 크거나 같고 len(grid[0])보다 작고, 지도에서 nrnc 위치가 벽이 아니고 방문한 적이 x, + 그리고 물이 아닐 때
                visited.add((nr, nc)) # 방문했다고 좌표 추가
                queue.append(((nr, nc), actions + [MOVE_CMDS[d]])) # 행동하고(방향) 움직임
    return []
...
M_FIRE_CMDS = {0: "R F M", 1: "D F M", 2: "L F M", 3: "U F M"}
START_SYMBOL = 'M' # 내 포탑 시작 위치
TARGET_SYMBOL = 'X' # 적 포탑
WALL_SYMBOL = 'R' # 바위
